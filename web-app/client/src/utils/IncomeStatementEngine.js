 ...
 */
export function generarEstadoResultados(accounts, options = {}) {
    const {
    ...
    if (utilidadBrutaEjercicio > 0) {
        ...
        if (utilidadNeta > 0 && debeAplicar) {
            const pct = porcentajeReservaLegal / 100;
            reservaLegal = utilidadNeta * pct;
        }
        ...
        logAudit.push("Resultado Negativo o Nulo: No se calculan impuestos.");
    }
 ...
 */
export async function generarEstadoResultadosDesdeWorksheet(companyId, options = {}) {
    try {
        // Obtener los mismos datos que usa Worksheet
        const [accountsRes, bcRes, adjRes, companyRes] = await Promise.all([
            fetch(`/api/accounts?companyId=${companyId}`).then(r => r.json()),
            fetch(`/api/reports/ledger?companyId=${companyId}&excludeAdjustments=true&excludeClosing=true`).then(r => r.json()),
            fetch(`/api/reports/ledger?companyId=${companyId}&adjustmentsOnly=true&excludeClosing=true`).then(r => r.json()),
